...
class ListItemProvider extends ChangeNotifier {
    ...
    item.child = Column(
      children: [
        !item.bottomPlaceholder!
            ? Container( ///placeholder when moving draggedCard around below it (shadow)
  ...
  void reorderCard() {
    ...
    // Update the UI to reflect the previous child state (optional)
    ...
    // Get the dragged card from the old list
    var draggedCard = boardProv.board.lists[boardProv.draggedItemState!.listIndex!].items
        .removeAt(boardProv.draggedItemState!.itemIndex!);

    // Handle the card reordering logic
    if (boardProv.move == 'LAST') {
      // Move card to the end of the new list
      boardProv.board.lists[boardProv.board.dragItemOfListIndex!].items.add(draggedCard);
    } else if (boardProv.move == "REPLACE") {
      // Clear the target list and insert the card
      boardProv.board.lists[boardProv.board.dragItemOfListIndex!].items.clear();
      boardProv.board.lists[boardProv.board.dragItemOfListIndex!].items.add(draggedCard);
    } else {
      // Move card to the calculated index in the target list
      int targetIndex = boardProv.board.dragItemIndex!;

      // Ensure we calculate the right target index when moving "DOWN"
      if (boardProv.move == "DOWN") {
        targetIndex = targetIndex - 1 < 0 ? 0 : targetIndex - 1;
      }

      // Insert the dragged card at the correct index
      boardProv.board.lists[boardProv.board.dragItemOfListIndex!].items.insert(targetIndex, draggedCard);
    }

    // Trigger the `onItemReorder` callback if defined
    if (boardProv.board.onItemReorder != null) {
      int oldCardIndex = boardProv.draggedItemState!.itemIndex!;
      int newCardIndex = boardProv.board.dragItemIndex!;
      int oldListIndex = boardProv.draggedItemState!.listIndex!;
      int newListIndex = boardProv.board.dragItemOfListIndex!;

      // Invoke the callback with old and new positions
    ...
    // Optionally notify listeners to refresh the UI
    notifyListeners();
  }
}
